...
async function createTempWorkspace() {
  ...
  await fs.mkdir(path.join(checkoutRoot, 'powerx/docs/standards'), { recursive: true });
...
test('push-standards copies standards and writes report', async () => {
  ...
  const copied = await fs.readFile(
    path.join(root, 'repos/powerx/docs/standards/README.md'),
    'utf8',
  );
  assert.match(copied, /Standards/);

  const shared = await fs.readFile(
    path.join(root, 'repos/powerx/docs/standards/_shared/common.md'),
    'utf8',
  );
  assert.match(shared, /Common/);

  const backendSpecific = await fs.readFile(
    path.join(root, 'repos/powerx/docs/standards/powerx/backend/backend.md'),
    'utf8',
  );
...
test('push-standards supports include filter', async () => {
  ...
  const sharedExists = await fs
    .stat(path.join(root, 'repos/powerx/docs/standards/shared.md'))
    .then(() => true)
    .catch(() => false);
  assert.equal(sharedExists, false);

  const backendSpecificExists = await fs
    .stat(path.join(root, 'repos/powerx/docs/standards/powerx/backend/backend.md'))
    .then(() => true)
    .catch(() => false);
  assert.equal(backendSpecificExists, false);

  const marketplaceSpecificExists = await fs
    .stat(path.join(root, 'repos/powerx-marketplace/docs/standards/powerx-marketplace/market.md'))
    .then(() => true)
    .catch(() => false);
...
test('push-standards respects standards map by scope', async () => {
  ...
  const marketplaceExists = await fs
    .stat(path.join(root, 'repos/powerx/docs/standards/powerx-marketplace/market.md'))
    .then(() => true)
    .catch(() => false);
