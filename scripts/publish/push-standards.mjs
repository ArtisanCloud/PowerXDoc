...
import { runGit, checkoutBranch, commitAll, ensureRemote, pushBranch } from '../lib/git-utils.mjs';
...
async function main() {
  ...
  for (const repoMeta of repositories) {
    ...
    try {
      await fs.stat(repoDir).catch(() => {
        throw new Error(
          `[checkout] Missing repository checkout at ${repoDir}. Clone ${repoMeta.git_url} first or set --checkout-root accordingly.`,
        );
      });
