...
```bash
npm run publish:usecases -- --scn-id SCN-PUBLISH-001 --dry-run
```

- 不会写入任何仓库，只生成 `reports/usecases/usecases_SCN-PUBLISH-001.json`。  
- 报告里包含分发目标、文件清单和 `resumeToken`。

## 2. 正式发布

```bash
npm run publish:usecases -- --scn-id SCN-PUBLISH-001
```
...
```bash
npm run publish:notify -- --scn-id SCN-PUBLISH-001
```
