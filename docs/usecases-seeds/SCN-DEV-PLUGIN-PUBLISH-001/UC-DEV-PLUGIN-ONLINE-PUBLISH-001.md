...
- **业务目标**：使供应商通过在线渠道完成插件发布与上架，确保签名、合规与元数据同步一致，并即时触达订阅租户。
- **成功度量**：在线发布成功率 ≥ 99%；审核 SLA ≤ 48 小时；通知延迟 ≤ 5 分钟；补件率 < 8%。
- **场景关联**：对应主场景 Stage 4，覆盖 Marketplace 审核与上架同步流程。

> 该用例通过 CLI/控制台联动审核管道，实现上线回执、订阅通知与运营报表的自动闭环。

# Context & Assumptions

- **前置条件**
  - Feature Flag `plugin-online-publish`、`marketplace-review-v2` 已启用。
  - 版本已通过测试与审批，产物签名有效，并存储在制品仓。
  - 开发者准备好更新日志、定价策略、支持窗口与合规声明。
  - 通知中心、审计日志、报表生成服务可用。
- **输入/输出**
  - 输入：发布制品引用、版本元数据、定价/支持信息、截图、合规文档。
...
| 层 | 主要组件/模块 | 责任 | 代码入口 |
|----|---------------|------|---------|
| 发布入口 | `packages/cli/src/commands/plugin/publish.ts` | 元数据收集、校验、提交申请、回执显示 | `packages/cli` |
...
| 发布记录 | `internal/publish/records/manager.go` | 版本对比、审计日志、通知触发、回执存档 | `services/publish/records` |
| 通知 & 报表 | `internal/notify/marketplace/listing.go` | 订阅通知、公告、初始报表生成 | `services/notify/marketplace` |

## 流程与时序

1. **Step 1 – 发布申请**：开发者执行 `px-plugin publish` 或通过控制台提交版本、定价、支持信息。
2. **Step 2 – 校验与补件**：审核管道自动执行签名、兼容、合规、安全校验，必要时生成补件任务。
3. **Step 3 – 审核决策**：审核员确认资料并出具结论，系统回传结果并记录审计。
4. **Step 4 – 上架与通知**：Marketplace 上架插件、生成公告、通知订阅租户并输出初始运营报表。
...
sequenceDiagram
  ...
  CLI->>Review: 提交制品与元数据
  Review-->>Review: 校验签名/兼容/合规
  Review->>CLI: 审核状态 & 补件请求
  Review->>Catalog: 更新上架状态
  Catalog->>Notify: 推送订阅通知
  Notify->>Dev: 上架回执 & 报表链接
```

# Contracts & Interfaces

- **Inbound APIs / Events**
  - `px-plugin publish`、`POST /marketplace/online/apply`。
  - `POST /marketplace/review/decision`。
- **Outbound 调用**
  - `POST /internal/security/signature/verify`、`POST /internal/compliance/review`。
  - `POST /internal/notify/marketplace/listing` — 通知订阅租户。
  - `POST /internal/analytics/marketplace/report` — 生成初始报表。
- **配置与脚本**
  - `config/marketplace/online_publish.yaml` — 元数据字段、校验规则。
  - `config/publish/metadata_template.json` — CLI 模板化提示。
  - `scripts/workflows/marketplace-online-publish.mjs` — E2E 验证脚本。
...
- **单元**：CLI 参数解析、元数据校验、审核决策状态机、通知模板。
- **集成**：运行 `scripts/workflows/marketplace-online-publish.mjs` 模拟成功与补件场景。
- **端到端**：复现 meta 用例 G（正向/逆向），验证签名失败、补件、快速上架。
...
- **回滚策略**：审核失败或上线异常时自动保留旧版本，支持一键下架、恢复订阅通知。
- **补救措施**：补件任务自动推送，CLI 提供 `px-plugin publish --resume`；通知失败则重试并发送人工提醒。
...
|-----------|------|----------|--------|-----|
| 部分市场定价策略差异导致审核延迟 | 区域上线进度 | 引入区域化模板与审批流程 | Ivy Chen | 2025-12-27 |
| CLI 提交过程缺乏并发保护 | 数据一致性 | 加入防重复提交 token 与乐观锁 | Alex Wei | 2025-12-19 |
| 通知通道拥塞影响延迟 | 租户触达 | 增加通知队列容量与监控告警 | Matrix Ops | 2025-12-24 |
