doc_id: PX-EXAMPLE-001              # 唯一子用例 ID（与 docmap.yaml、下游路径一致）
scn_id: SCN-EXAMPLE-001             # 关联主用例 ID
title: PX (service) - 示例子用例标题  # 面向读者的标题，可包含层/域信息
status: Draft                       # Draft | In Review | Approved | Deprecated
version: v0.1.0                     # 模板版本，便于比较与审计
repo_key: powerx-backend            # 对应 docs/_data/repos.yaml 的 key
scope: powerx                       # 目录 scope，决定 usecases-seeds/<scope>/...
layer: service                      # 体系层：proto | api | service | repo | ui | ops | ...
domain: publish                     # 业务域：publish | install | billing | ...
owners:
...
linked_requirements:
- id: RQ-1234
    description: 示例：关联的需求、OKR 或缺陷编号
code_refs:
- path: backend/services/catalog-sync
    description: 主要实现入口或模块路径
feature_flags: []
optional: false
last_reviewed_at: 2025-01-01        # yyyy-mm-dd，最近人工审查时间
