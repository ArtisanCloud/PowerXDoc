...
| 模块 | 主要组件/模块 | 责任 | 代码入口 |
|------|---------------|------|---------|
| WatchPipeline | `cli/src/commands/dev/watch.ts` | 解析配置、初始化 watcher、触发增量构建 | `packages/cli/src/index.ts` |
| BundleBuilder | `cli/src/build/bundle.ts` | 调用 esbuild/tsc、生成差异产物、校验大小与签名 | `packages/cli/src/build` |
| SessionClient | `cli/src/runtime/hotreload/session.ts` | 负责 Dev API register/reload/delete 调用、重试与 mTLS | `packages/cli/src/runtime` |
| TelemetryEmitter | `cli/src/telemetry/dev.ts` | 采集构建与 API 延迟，写入 Workflow Metrics/Kafka | `packages/cli/src/telemetry` |
...
- **Dev API 调用**（由 SessionClient 发起）
  - `POST /internal/dev/plugins/register`：携带 manifest、bundleMeta、cliVersion、traceId。
  - `POST /internal/dev/plugins/reload`：携带 sessionId、reloadToken、`changedFiles`、`diagnostics`；支持 `x-reload-id` 幂等头。
- **配置文件**
...
| 项目 | 描述 | 完成状态 | 负责人 |
|------|------|----------|--------|
| watch 入口 | 支持 `--watch` 共享配置、ESM/CJS 插件解析 | [ ] | Li Wei |
| 增量构建 | diff 计算、hash 方案、超大文件跳过策略 | [ ] | Li Wei |
| Dev API 客户端 | mTLS、重试、Backoff、限速 | [ ] | Carol |
| Telemetry | `dev.hotload.cli_build_time_ms`、`dev.hotload.cli_error` | [ ] | Matrix-X |
| 自动校验 | `px-plugin lint --hotload` 校验 manifest 与脚本 | [ ] | Li Wei |
| 文档 | 更新 `README.dev.md`、`docs/guides/dev-hotload.md` | [ ] | Matrix-X |
...
|-----------|------|----------|--------|-----|
| 大型仓库 watch 过多导致内存暴涨 | CLI OOM | 默认忽略 build 目录，支持 `--watch-max-files` 并提示配置 | Li Wei | 2025-02-05 |
| mTLS 证书更新导致 CLI 无法注册 | 调试中断 | `px auth rotate` 自动刷新；CLI 检查证书有效期提前 7 天提醒 | Carol | 2025-02-12 |
| Telemetry 不可用影响调试指标 | 观测盲区 | CLI 支持本地缓冲/重放；告警 CLI Telemetry 失联 | Matrix-X | 2025-02-08 |
| manifest schema 变更未同步 | 发布链路失败 | 提前拉取 Marketplace schema，CI 校验并提示升级 | Li Wei | 2025-02-01 |
