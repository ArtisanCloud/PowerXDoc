...
- **集成**：运行 `scripts/workflows/plugin-local-dev-smoke.mjs` 验证链路；模拟依赖缺失、权限不足、签名失败。
- **端到端**：复现 meta 用例 E（正向/逆向），确保日志、审计、权限提示正常。
- **非功能**：并行调试多个插件、长时间 watch 稳定性、跨平台兼容性。
...
- **补救措施**：提供 `px-plugin dev --reset` 清理缓存；支持一键导出日志和权限建议。
...
| 权限提示与生产策略存在差异 | 上线后仍可能缺权限 | 统一权限模板生成逻辑 | Grace Lin | 2025-12-21 |
...
- 脚本：`scripts/workflows/plugin-local-dev-smoke.mjs`
