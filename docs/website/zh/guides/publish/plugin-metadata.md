...
| `dependencies.requires` | `compat.requires` | 影响兼容性矩阵，发布前逐项核对 |

> `px-plugin manifest sync` / `px-plugin metadata lint` 尚未在当前 CLI 中发布，需手动同步上述字段。
...
1. **静态校验**  
   - 使用 IDE、`yamllint` 等工具检查语法。
   - 按 `plugin_yaml_spec.md` 附录逐项确认 ID、版本、entry、权限等字段。
2. **字段对齐**  
   - 用 `diff` 或 IDE 比对 `plugin.yaml` / `manifest.yaml` 的菜单、权限、版本号。
3. **依赖锁定**  
