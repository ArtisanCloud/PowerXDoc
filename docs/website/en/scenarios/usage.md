...
- **Preview**: Run `node scripts/site/sync-scenario-pages.mjs --scn-id <SCN_ID>` to render the Markdown into `docs/website/{zh,en}/scenarios/` for review.
...
- **Command**: `node .specify/scripts/node/setup-usecase-seeds.mjs --scn-id <SCN_ID>`
...
- **Command**: `node .specify/scripts/node/generate-usecase-seed-index.mjs --scn-id <SCN_ID>`
...
- **Single scenario**: `npm run publish:usecases -- --scn-id <SCN_ID>`
